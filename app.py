...
def display_side_by_side(original, processed, title_original="Original Image", title_processed="Processed Image"):
    ...
    if len(original.shape) == 2:  # Grayscale
        ...
        ax2.imshow(cv2.cvtColor(processed, cv2.COLOR_BGR2RGB) if len(processed.shape) == 3 else processed,
                   cmap='gray' if len(processed.shape) == 2 else None)
...
if uploaded_file is not None:
    ...
    if technique == "Thresholding":
        ...
        if operation == "Resize":
            ...
            x, y = st.slider("Center X", 0, img.shape[1], img.shape[1] // 2), st.slider("Center Y", 0, img.shape[0],
                                                                                        img.shape[0] // 2)
